...
def autolimits(data, err=None):
    """Returns sensible x or y limits for a plot containing data as positions. Can deal with data as ufloatarrays."""
    if isinstance(data[0], uncertainties.UFloat):
        data = uncertainties.nominal_value(data)
        err = uncertainties.std_dev(data)
